...
var firebase_1 = require("../../utils/Functions/firebase");
...
function Create(_a) {
    ...
    var _e = react_1.useState(new Array(1000).fill(true)), isMcq = _e[0], setIsMcq = _e[1];
    ...
    var _q = react_1.useState(), image = _q[0], setImage = _q[1];
    var addQuestion = function (i) {
        if (id) {
            setEditQuestions(function (prev) { return __spreadArrays(prev, [{ question: "", isMcq: false }]); });
        }
        setQuestions(function (prev) { return __spreadArrays(prev, [{ question: "", isMcq: false }]); });
    };
    ...
    var handleImage = function (e) {
        ...
        setPosts(function (prev) { return (__assign(__assign({}, prev), { ulid: ulid_1.ulid() })); });
        console.log(posts);
    ...
    var questionType = function (i) {
        ...
        if ((id && editPosts && editPosts.questions && editPosts.questions[i] && editPosts.questions[i].isMcq)) {
            var arr = { questions: editQuestions[i].question, isMcq: false };
            console.log(arr);
            setEditQuestions(function (prev) { return prev.map(function (p, j) { return j === i ? { question: p.question, isMcq: false } : p; }); });
            // setEditPosts( ( prev ) => ( { ...prev, questions: editQuestions } ) )
            // console.log( editQuestions, editPosts )
            forceUpdate();
        }
        if ((posts && editPosts.questions && editPosts.questions[i] && editPosts.questions[i].isMcq)) {
            var arr = { questions: editQuestions[i].question, isMcq: false };
            console.log(arr);
            setEditQuestions(function (prev) { return prev.map(function (p, j) { return j === i ? { question: p.question, isMcq: false } : p; }); });
            // setEditPosts( ( prev ) => ( { ...prev, questions: editQuestions } ) )
            ...
            if (id) {
                var arr_1 = { question: editQuestions[i].question, options: [{
                            value: "",
                            answer: false
                        }], isMcq: true };
                console.log(arr_1);
                setEditQuestions(function (prev) {
                    var array = prev.map(function (p, j) {
                        return j === i ? arr_1 : p;
                    });
                    console.log(array);
                    return array;
                ...
                var arr_2 = { question: questions[i].question, options: [{
                            value: "",
                            answer: false
                        }], isMcq: true };
    ...
    var submit = function () {
        if (id && image) {
            firebase_1["default"].ref("/images/" + editPosts.ulid).put(image).on("state_changed", function (snapshot) { return console.log(snapshot); }, alert, function () {
                firebase_1["default"].ref("images").child(editPosts.ulid).getDownloadURL().then(function (url) { return editPosts.image = url; }).then(function () {
        ...
        firebase_1["default"].ref("/images/" + posts.ulid).put(image).on("state_changed", function (snapshot) { return console.log(snapshot); }, alert, function () {
            console.log(posts.ulid);
            firebase_1["default"].ref("images").child(posts.ulid).getDownloadURL().then(function (url) { return posts.image = url; }).then(function () {
                console.log(posts);
    ...
    react_1.useEffect(function () {
        ...
        if (settingDone && changeDone) {
            ...
            setEditQuestions(function (prev) { return prev.map(function (p, i) { return i === currentIndex ? object_1 : p; }); });
            // console.log( editPosts )
            return;
        }
        if (!id) {
            ...
            setQuestions(function (prev) { return prev.map(function (p, i) { return i === currentIndex ? object_2 : p; }); });
            // setPosts( posts )
            // console.log( posts )
        }
    }, [mcq, settingDone]);
    ...
    return (react_1["default"].createElement(react_1["default"].Fragment, null, allPosts && ((id && (typeof editPosts.name === 'string')) || (!id)) && (react_1["default"].createElement(react_1["default"].Fragment, null,
        react_1["default"].createElement(material_1.Container, { className: "my-4" },
            react_1["default"].createElement("h3", { className: "my-4" },
                " ",
                editPosts.name || posts.name ? (id ? "Edit " + editPosts.name : "Create " + posts.name) : "Create Quiz",
                " "),
            next ? (react_1["default"].createElement(material_1.Box, { sx: {
                ...
                (id ? editQuestions : questions).map(function (question, i) { return (react_1["default"].createElement("div", { onChange: function () { setCurrentIndex(i); setChangeDone(true); }, key: i, className: "textBox" },
                    ...
                    ((id && editPosts && editPosts.questions && editPosts.questions[i] && editPosts.questions[i].isMcq) || (posts && posts.questions && posts.questions[i] && posts.questions[i].isMcq))
                        ? (react_1["default"].createElement("div", { className: "mb-8 w-full" }, id && editPosts.questions[i] ? react_1["default"].createElement(mcq_1["default"], { changer: [1, 2], mcqData: mcqData, preData: editPosts.questions[i].options }) : react_1["default"].createElement(mcq_1["default"], { changer: [1, 2], mcqData: mcqData }))) : (react_1["default"].createElement(react_1["default"].Fragment, null, " ")))); }),
